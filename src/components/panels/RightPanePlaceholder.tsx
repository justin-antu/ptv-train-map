 ...
 * then a quiet "coming soon" teaser (static dot-pattern backdrop + dashed
 * preview tiles) for the still-reserved remainder of the space.
 *
 * The dot pattern is deliberately static (no `glow` animation): a glowing
 * dot grid animates every individual dot continuously via Motion, which
 * adds up to real, ongoing CPU cost for a purely decorative background.
 */
export const RightPanePlaceholder = memo(function RightPanePlaceholder({ stats }: RightPanePlaceholderProps) {
  return (
    <div className="relative flex h-full flex-col overflow-hidden">
      ...
      <div className="relative flex flex-1 flex-col gap-4 overflow-y-auto p-3">
        ...
        <div className="flex flex-1 flex-col items-center justify-center gap-4 p-3 text-center">
          <span className="rounded-full bg-primary/10 px-4 py-1.5 text-[11px] font-semibold tracking-wide text-primary uppercase">More on the way</span>
